...
while (i < 20) {
    document.write(`<h1>Div number ${i + 1}</h1>`);
    i++;
}
...
while (j < 20) {
    document.write(`<h1>Div index ${j}</h1>`);
    j++;
}
...
for (const user of users) {
    user.status ? console.log(`${user.name} ${user.status} and ${user.age}`) : '';
}
...
for (const user of users) {
    user.age > 30 ? console.log(`${user.name} is ${user.age}`) : '';
}
